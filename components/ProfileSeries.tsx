...
interface ProfileFilmsProps{
  id:string;
  serializedSeries:WatchedProps[];
}

const ProfileSeries = ({id,serializedSeries}:ProfileFilmsProps) => {
  ...
  const mediaIds = serializedSeries.map((film) => film.mediaId)
  ...
  const fetchMovieData = async () =>{
      try {
        ...
        const moviesWithCreatedAt = movieData.map((movie, index) => ({
          ...
          createdAt: serializedSeries[index].createdAt,
        }));  
